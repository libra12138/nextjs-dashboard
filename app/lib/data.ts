...
export async function fetchLatestInvoices() {
  noStore();
  try {
    const data: LatestInvoiceRaw[] = await db.$queryRaw`
...
export async function fetchFilteredInvoices(
  ...
  try {
    const invoices: InvoicesTable[] = await db.$queryRaw`
      SELECT
        invoices.id,
        invoices.amount,
        invoices.date,
...
export async function fetchInvoiceById(id: string) {
  noStore();
  try {
    const data: InvoiceForm[] = await db.$queryRaw`
      SELECT
        invoices.id,
...
export async function fetchCustomers() {
  try {
    const data: CustomerField[] = await db.$queryRaw`
      SELECT
        id,
        name
      FROM customers
...
export async function fetchFilteredCustomers(query: string) {
  try {
    const data: CustomersTableType[] = await db.$queryRaw`
		SELECT
		  customers.id,
		  customers.name,
		  customers.email,
